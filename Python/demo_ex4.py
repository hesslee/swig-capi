...
try:
    ...
    rc = capi.altibase_stmt_prepare(stmt, "INSERT INTO DEMO_EX4 VALUES( ?, ?, ?, ?, ?, ? )")
    if ( rc != capi.ALTIBASE_SUCCESS ):
        raise stmtError("altibase_stmt_prepare")

    array_size = 10
    capi.altibase_stmt_set_array_bind(stmt, array_size);
    if ( rc != capi.ALTIBASE_SUCCESS ):
        raise stmtError("altibase_stmt_prepare")
    ...
    # column binding : id
    capi.apx_arraybind_string(array_size, bind, 0, 8)
    # column binding : name
    capi.apx_arraybind_string(array_size, bind, 1, 20)
    # column binding : age
    capi.apx_arraybind_int(array_size, bind, 2)
    # column binding : birth
    capi.apx_arraybind_date(array_size, bind, 3)
    # column binding : sex
    capi.apx_arraybind_short(array_size, bind, 4)
    # column binding : etc
    capi.apx_arraybind_double(array_size, bind, 5)
    ...
    for i in range(10):
        # column binding : id
        capi.apx_arraybind_put_string(i, bind, 0, str(i) + "0000000")
        # column binding : name
        capi.apx_arraybind_put_string(i, bind, 1, "name" + str(i))
        # column binding : age
        capi.apx_arraybind_put_int(i, bind, 2, i)
        # column binding : birth
        capi.apx_arraybind_put_date(i, bind, 3, 1980 + i, 10, 10, 8, 50, 10, 0)
        # column binding : sex
        capi.apx_arraybind_put_short(i, bind, 4, i % 2)
        # column binding : etc
        if ( i % 2 == 0 ):
            capi.apx_arraybind_put_double(i, bind, 5, 10.2 + i)
        else:
            capi.apx_arraybind_put_null(i, bind, 5)
    ...
    rc = capi.altibase_stmt_prepare(stmt, "SELECT * FROM DEMO_EX4")
    if ( rc != capi.ALTIBASE_SUCCESS ):
        raise stmtError("altibase_stmt_prepare")
